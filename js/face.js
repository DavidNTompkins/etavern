...
class FaceMeshFaceGeometry extends BufferGeometry {
  constructor(options = {}) {
    ...
    this.applyMatrix4(new Matrix4().makeScale(10, 10, 10)); //DT - changed from 10 10 10 to 2 2 2 (reverted)
    this.p0 = new Vector3();
    this.p1 = new Vector3();
    this.p2 = new Vector3();
  ...
  update(face, cameraFlipped) {
    let ptr = 0;
    for (const p of face) { //removed .scaledMesh after face as only passing in the scaledMesh now
    ...
    if (this.useVideoTexture) {
      ...
      if (this.normalizeCoords) {
        ...
        for (const p of face) { //same as above.
